from cs50 import SQL

db = SQL("sqlite:///gamers.db")


def get_usernames():
    sql = "SELECT player_id, username FROM players ORDER BY username"
    rows = db.execute(sql)
    return rows

def get_all_games_by_title():
    sql = "SELECT game_id, title FROM games ORDER BY title"
    return db.execute(sql)

def add_history(player_id, game_id, score, date_played):
    sql = """INSERT INTO player_history (player_id, game_id, score, date_played)
    VALUES (?, ?, ?, ?)"""
    hist_id = db.execute(sql, player_id, game_id, score, date_played)
    return hist_id

def get_history_by_player(player_id):
    sql = """SELECT username, title, score, date_played FROM players
        JOIN player_history USING (player_id)
        JOIN games USING (game_id)
        WHERE player_id = ?
        ORDER BY date_played DESC"""
    rows = db.execute(sql, player_id)
    return rows


def get_all_players():
    sql = "SELECT * FROM players ORDER BY join_date DESC"
    players = db.execute(sql)
    # select statements return a list of dictionaries
    return players

def add_player(username, email):
    sql = "INSERT INTO players (username, email) VALUES (?, ?)"
    try:
        pk = db.execute(sql, username, email)
        # insert statements return the primary key of the new row
        return pk
    except ValueError as e:
        return -99


def get_player_by_id(p_id):
    sql = "SELECT * FROM players WHERE player_id = ?"
    players = db.execute(sql, p_id)
    if len(players) != 1:
        return False
    else:
        # should be a list of 1 player so return that dictionary
        return players[0]


def validate_price(price):
    # the price needs to be a number that is >= 0
    try:
        price = float(price)
    except ValueError as e:
        return -1
        # return render_template('error.html', message=f'Sorry {price} is not a number')
    except:
        return -2
        # return render_template('error.html', message='Sorry somethin else went wrong')
    else:
        if price >= 0:
            return price
        else:
            return -3



def get_player_email(id):
    sql = "SELECT email FROM players WHERE player_id = ?"
    rows = db.execute(sql, id)
    print(rows)
    # we should get only one row
    if len(rows) == 1:
        return rows[0].get('email')
    else:
        return None

def email_is_unique(new_email):
    sql = "SELECT email FROM players WHERE email = ?"
    rows = db.execute(sql, new_email)
    if not rows:
        return True
    else:
        return False

def update_player(player_info, id):
    sql = "UPDATE players SET username = ?, email = ?, join_date = ? WHERE player_id = ?"
    number_updated = db.execute(sql,
                                player_info.get('username'),
                                player_info.get('email'),
                                player_info.get('join_date'),
                                id)
    return number_updated


def delete_player(id):
    sql = "DELETE FROM players WHERE player_id = ?"
    number_deleted = db.execute(sql, id)
    return number_deleted

